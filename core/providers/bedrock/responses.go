	...
	for i, msg := range bifrostMessages {
		...
		if msgType == schemas.ResponsesMessageTypeMessage && msg.Role != nil &&
			*msg.Role != schemas.ResponsesInputMessageRoleSystem && *msg.Role != schemas.ResponsesInputMessageRoleDeveloper {
			...
			var err error
			convertedRegularMessage, disposition, err = convertBifrostMessageToBedrockMessageWithDisposition(ctx, &msg)
			if err != nil {
				return nil, nil, err
			}
...
func convertBifrostMessageToBedrockMessage(ctx context.Context, msg *schemas.ResponsesMessage) (*BedrockMessage, error) {
	bedrockMsg, _, err := convertBifrostMessageToBedrockMessageWithDisposition(ctx, msg)
				...
				if file := block.ResponsesInputMessageContentBlockFile; file != nil {
					...
					if file.FileURL != nil {
						if s3Loc, ok := bedrockS3LocationFromURL(*file.FileURL); ok {
							...
							if format == "" {
								return bedrockResponsesContentConversion{Disposition: bedrockResponsesContentUnsupported}, fmt.Errorf("cannot determine document format for %q: set file_type or give the object a file extension", *file.FileURL)
							}
							...
							return bedrockResponsesContentConversion{Disposition: bedrockResponsesContentUnsupported}, fmt.Errorf("invalid s3:// document reference %q: expected s3://bucket/key", *file.FileURL)
						}
					}
					if format != "" {
						doc.Format = format
