...
const RichTextarea = ({
	...
	onChange,
	highlightPatterns,
	suggestionsTrigger: _customCompletionsTrigger,
	textAreaClassName,
	noSuggestionText,
	...
	const getNestedSuggestions = useCallback(
		(path: string[]): DropdownOption[] | undefined => {
	...
	}, [currentPath, getNestedSuggestions, searchText]);
	...
	useEffect(() => {
		const handleWindowClick = (event: MouseEvent) => {
			...
			if (containerRef.current && !containerRef.current.contains(event.target as Node) && !dropdownPortal?.contains(event.target as Node)) {
				setShowDropdown(false);
				setSearchText("");
				setCurrentPath([]);
			}
							...
							(() => {
								...
								return value.split(highlightPattern).map((part, i) => {
									...
									return (
										<mark
											key={i}
											className={cn("rounded-sm pb-0.5 italic outline", className)}
											onClick={
												enableVariableClickEdit && matchedPattern.enableVariableClickEdit
													? (event) => handleVariableClick(event, currentPartPosition, currentPartPosition + part.length, part)
													: undefined
											}
										>
											{part}
