...
export class RoutingRulesPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.createBtn = page
      ...
      .or(page.getByRole("button", { name: /New Rule|Create First Rule/i }).first());
    ...
    this.saveBtn = page
      ...
      .or(page.locator('[role="dialog"]').getByRole("button", { name: /Save Rule|Update Rule/i }));
    // Cancel button specifically (not the Close/X button in header)
    this.cancelBtn = page
      .locator('[data-testid="cancel-rule-btn"]')
      .or(page.locator('[role="dialog"]').getByRole("button", { name: "Cancel", exact: true }));
  }
  ...
  async createRoutingRule(config: RoutingRuleConfig): Promise<void> {
    ...
    if (config.provider) {
      const providerCombo = this.sheet
        .getByRole("combobox")
  ...
  async selectRuleOperator(ruleIndex: number, operatorName: string): Promise<void> {
    ...
    const option = this.page
      .getByRole("option")
      .filter({ hasText: new RegExp(`^${operatorName}$|^${operatorName} `, "i") })
      .first();
    await option.click({ force: true });
    await this.page.waitForTimeout(300);
  }
  ...
  async addFallbackProvider(provider: string, model?: string): Promise<void> {
    ...
    const addFallbackBtn = this.sheet
      .getByRole("button", { name: /Add Fallback/i })
      .or(this.sheet.locator("button").filter({ hasText: /Fallback/i }));
    ...
    if (isVisible) {
      ...
      const fallbackInput = this.sheet
        .locator('input[placeholder*="fallback" i], input[placeholder*="provider" i]')
        .first();
