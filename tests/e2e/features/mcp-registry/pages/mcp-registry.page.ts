...
export class MCPRegistryPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.createBtn = page
      .locator('[data-testid="create-mcp-client-btn"]')
      .or(
        page
    ...
    this.nameInput = page
      ...
      .or(this.sheet.getByLabel(/Name/i).first());
    this.saveBtn = page
      .locator('[data-testid="save-client-btn"]')
      .or(
        this.sheet
          .getByRole("button", { name: /Create/i })
          .or(this.sheet.getByRole("button", { name: /Save/i })),
      );
    this.cancelBtn = page
      .locator('[data-testid="cancel-client-btn"]')
      .or(this.sheet.getByRole("button", { name: /Cancel/i }));
    ...
    this.connectionUrlInput = this.sheet
      .getByPlaceholder(/http:\/\/your-mcp-server/i)
      .or(page.locator('[data-testid="connection-url-input"]'));
    ...
    // OAuth inputs
    this.oauthClientIdInput = this.sheet
      .locator('[data-testid="mcp-oauth-client-id"]')
      .or(this.sheet.getByPlaceholder(/your-client-id/i));
    this.oauthClientSecretInput = this.sheet
      .locator('[data-testid="mcp-oauth-client-secret"]')
      .or(this.sheet.getByPlaceholder(/your-client-secret/i));
    this.oauthAuthorizeUrlInput = this.sheet
      .locator('[data-testid="mcp-oauth-authorize-url"]')
      .or(this.sheet.getByPlaceholder(/oauth\/authorize/i));
    this.oauthTokenUrlInput = this.sheet
      .locator('[data-testid="mcp-oauth-token-url"]')
      .or(this.sheet.getByPlaceholder(/oauth\/token/i));
    this.oauthScopesInput = this.sheet
      .locator('[data-testid="mcp-oauth-scopes-input"]')
      .or(this.sheet.getByPlaceholder(/read, write, admin/i));
  }
  ...
  async fillClientForm(config: MCPClientConfig): Promise<void> {
    ...
    if (config.isCodeMode !== undefined) {
      ...
      if (currentState !== config.isCodeMode) {
        ...
        await expect(this.codeModeSwitch).toHaveAttribute("data-state", expectedState, {
          timeout: 3000,
        });
      }
    }
        ...
        for (let i = 0; i < entries.length; i++) {
          ...
          const valueStr =
            typeof val === "object" && val !== null && "value" in val
              ? (val as EnvVarLike).value
              : String(val);
          const keyInput = headersTable.locator(`input[data-row="${i}"][data-column="key"]`);
          const valueInput = headersTable
            .locator(`input[data-row="${i}"][data-column="value"]`)
            .or(headersTable.locator(`[data-row="${i}"][data-column="value"] input`));
  ...
  async createClient(config: MCPClientConfig): Promise<boolean> {
    ...
    const responsePromise = this.page.waitForResponse(
      (response) => {
        ...
        return (
          url.includes("/mcp/client") &&
          !url.endsWith("/mcp/clients") &&
  ...
  async closeDetailSheet(): Promise<void> {
    ...
    await this.page.keyboard.press("Escape");
    await expect(this.detailSheet)
      .not.toBeVisible({ timeout: 5000 })
      .catch(async () => {
        // If still visible, try clicking X button
        const closeBtn = this.detailSheet
          .locator("button")
  ...
  async reconnectClient(name: string): Promise<void> {
    ...
    const reconnectDialog = this.page
