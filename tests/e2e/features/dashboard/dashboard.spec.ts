...
test.describe("Dashboard", () => {
  ...
  test.describe("Dashboard Display", () => {
    ...
    test("should display date time picker", async ({ dashboardPage }) => {
      ...
      const datePicker = dashboardPage.page
        .locator("button")
        .filter({ hasText: /Last/i })
        .or(dashboardPage.page.locator('[data-testid="dashboard-date-picker"]'));
  ...
  test.describe("Chart Data Validation", () => {
    test("should render chart elements after data loads", async ({ dashboardPage }) => {
      ...
      const volumeChartContent = dashboardPage.logVolumeChart.locator(
        "canvas, svg.recharts-surface",
      );
      const tokenChartContent = dashboardPage.tokenUsageChart.locator(
        "canvas, svg.recharts-surface",
      );
      const costChartContent = dashboardPage.costChart.locator("canvas, svg.recharts-surface");
      const modelChartContent = dashboardPage.modelUsageChart.locator(
        "canvas, svg.recharts-surface",
      );
  ...
  test.describe("Chart Interactions", () => {
    ...
    test("should sync model filter between cost and usage charts", async ({ dashboardPage }) => {
      ...
      const usageFilterVisible = await dashboardPage.usageModelFilter
        .isVisible()
        .catch(() => false);
  ...
  test.describe("Custom Date Range", () => {
    test("should open custom date range picker", async ({ dashboardPage }) => {
      ...
      if (isVisible) {
        ...
        const calendarVisible = await dashboardPage.page
          .locator('[role="dialog"], [role="listbox"]')
          .isVisible()
          .catch(() => false);
        const optionsVisible = await dashboardPage.page
          .getByRole("option")
          .first()
          .isVisible()
          .catch(() => false);
    ...
    test("should handle empty data for custom range", async ({ dashboardPage }) => {
      ...
      const errorAlert = dashboardPage.page.locator(
        '[role="alert"][data-variant="destructive"], .text-destructive, [data-sonner-toast][data-type="error"]',
      );
