...
export class DashboardPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.volumeChartToggle = page
      .locator('[data-testid="chart-log-volume-actions"]')
      .locator("button")
      .filter({ has: page.locator("svg") });
    this.tokenChartToggle = page
      .locator('[data-testid="chart-token-usage-actions"]')
      .locator("button")
      .filter({ has: page.locator("svg") });
    ...
    this.costChartToggle = page
      .locator('[data-testid="chart-cost-total-actions"]')
      .locator("> div > div")
      .last()
      .locator("button");
    this.modelChartToggle = page
      .locator('[data-testid="chart-model-usage-actions"]')
      .locator("> div > div")
      .last()
      .locator("button");
    ...
    this.costModelFilter = page
      .locator('[data-testid="chart-cost-total-actions"]')
      .locator('[role="combobox"], [data-slot="select-trigger"]')
      .first();
    this.usageModelFilter = page
      .locator('[data-testid="chart-model-usage-actions"]')
      .locator('[role="combobox"], [data-slot="select-trigger"]')
      .first();
  }
  ...
  async selectTimePeriod(period: "1h" | "6h" | "24h" | "7d" | "30d"): Promise<void> {
    ...
    await this.page
      .waitForSelector("[data-radix-popper-content-wrapper]", { timeout: 5000 })
      .catch(() => {});
  ...
  async isChartLoading(chartTitle: string): Promise<boolean> {
    ...
    const skeleton = chartCard
      .locator('[data-testid="skeleton"]')
      .or(chartCard.locator(".skeleton"));
    return await skeleton.isVisible().catch(() => false);
  }
  ...
  async getChartToggleState(toggle: Locator): Promise<string | null> {
    ...
    const isActive = await firstToggle
      .evaluate((el) => el.hasAttribute("active"))
      .catch(() => false);
    if (isActive) {
      return "active";
    }
