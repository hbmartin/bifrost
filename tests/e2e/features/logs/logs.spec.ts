...
test.describe("LLM Logs", () => {
  ...
  test.describe("Pagination", () => {
    ...
    test("should navigate to previous page", async ({ logsPage }) => {
      ...
      await expect.poll(() => logsPage.getCurrentPageNumber(), { timeout: 5000 }).toBe(2);
      ...
      await expect.poll(() => logsPage.getCurrentPageNumber(), { timeout: 5000 }).toBe(1);
    });
  });
  ...
  test.describe("Empty State", () => {
    test("should show empty state when no logs", async ({ logsPage }) => {
      ...
      await expect(logsPage.page.locator("text=/No results found|No logs found/i")).toBeVisible({
        timeout: 10000,
      });
    });
  });
  ...
  test.describe("URL State Persistence", () => {
    test("should persist filters in URL", async ({ logsPage }) => {
      ...
      await expect
        .poll(() => logsPage.page.url(), { timeout: 8000, intervals: [300, 500, 500] })
