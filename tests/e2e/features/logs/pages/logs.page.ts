...
export class LogsPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.statsCards = page
      .locator('[data-testid="stats-cards"]')
      .or(page.locator("text=Total Requests").locator("..").locator(".."));

    // Filter elements - filters are inside a popover opened by the Filters button
    this.providerFilter = page
      .locator('[data-testid="filter-provider"]')
      .or(page.locator("button").filter({ hasText: /Provider/i }));
    this.modelFilter = page
      .locator('[data-testid="filter-model"]')
    ...
    this.searchInput = page
      ...
      .or(page.getByPlaceholder("Search logs"));
  ...
  async filterByStatus(status: "success" | "error" | "pending"): Promise<void> {
    ...
    await this.page
      .waitForSelector('[role="listbox"], [data-slot="command-list"]', { timeout: 5000 })
      .catch(() => {});
  ...
  async goToNextPage(): Promise<void> {
    ...
    const limit =
      Number.parseInt(new URL(this.page.url()).searchParams.get("limit") ?? "25", 10) || 25;
