...
export class ModelLimitsPage extends BasePage {
  ...
  getModelLimitRow(modelName: string, provider: string = "all"): Locator {
    return this.page.getByTestId(
      `model-limit-row-${toTestIdPart(modelName)}-${toTestIdPart(provider)}`,
    );
  }
  ...
  async createModelLimit(config: ModelLimitConfig): Promise<string> {
    ...
    const targetOption = this.page
      .getByRole("option", { name: config.modelName, exact: true })
      .first();
    ...
    if (config.rateLimit?.requestMaxLimit !== undefined) {
      await this.page
        .locator("#modelRequestMaxLimit")
        .fill(String(config.rateLimit.requestMaxLimit));
    }
    ...
    await expect(this.getModelLimitRow(selectedModelName, config.provider)).toBeVisible({
      timeout: 15000,
    });
    return selectedModelName;
  }

  async editModelLimit(
    modelName: string,
    provider: string,
    updates: Partial<ModelLimitConfig>,
  ): Promise<void> {
    await this.openModelLimitActions(modelName, provider);
    const editBtn = this.page.getByTestId(
      `model-limit-button-edit-${toTestIdPart(modelName)}-${toTestIdPart(provider)}`,
    );
  ...
  async deleteModelLimit(modelName: string, provider: string = "all"): Promise<void> {
    ...
    const deleteBtn = this.page.getByTestId(
      `model-limit-button-delete-${toTestIdPart(modelName)}-${toTestIdPart(provider)}`,
    );
