...
export type ObservabilityConnector =
  | "otel"
  | "prometheus"
  | "maxim"
  | "datadog"
...
export class ObservabilityPage extends BasePage {
  ...
  private static readonly CONNECTOR_TOGGLE_TESTIDS: Partial<
    Record<ObservabilityConnector, string>
  > = {
  ...
  private static readonly CONNECTOR_DELETE_TESTIDS: Partial<
    Record<ObservabilityConnector, string>
  > = {
  ...
  getConnectorToggle(connector: ObservabilityConnector): Locator {
    ...
    return testId
      ...
      : this.page.locator('button[role="switch"]').first();
  }
  ...
  getConnectorDeleteBtn(connector: ObservabilityConnector): Locator {
    ...
    return testId
      ? this.page.getByTestId(testId)
      : this.page.locator('[data-testid="connector-delete-unused"]');
  }
  ...
  async selectConnector(connector: ObservabilityConnector): Promise<void> {
    ...
    const isDisabled =
      (await tab.getAttribute("aria-disabled")) === "true" || (await tab.isDisabled());
  ...
  async getSelectedConnector(): Promise<string | null> {
    ...
    const selected = this.page.locator(
      '[data-testid^="observability-provider-btn-"][aria-current="page"]',
    );
  ...
  async getCurrentState(): Promise<ObservabilityState> {
    ...
    const connectors: ObservabilityConnector[] = [
      "otel",
      "prometheus",
      "maxim",
      "datadog",
  ...
  /** Restore the enabled state captured before a test changed any connector. */
  async restoreState(state: ObservabilityState): Promise<void> {
    const desired: Record<ObservabilityConnector, boolean> = {
      otel: state.otelEnabled,
      prometheus: state.prometheusEnabled,
      maxim: state.maximEnabled,
      datadog: state.datadogEnabled,
      bigquery: state.bigqueryEnabled,
      newrelic: state.newRelicEnabled,
    };
    for (const connector of Object.keys(desired) as ObservabilityConnector[]) {
      if (!(await this.isConnectorAvailable(connector))) continue;
      await this.selectConnector(connector);
      if (
        (await this.isConnectorEnabled(connector)) !== desired[connector] &&
        (await this.isToggleEnabled(connector))
      ) {
        await this.toggleConnector(connector);
        if (await this.saveBtn.isEnabled().catch(() => false)) {
    ...
    for (const connector of connectors) {
      if (await this.isConnectorAvailable(connector)) {
        try {
          await this.selectConnector(connector);
          if (
            (await this.isConnectorEnabled(connector)) &&
            (await this.isToggleEnabled(connector))
          ) {
            await this.toggleConnector(connector);
            ...
            if (saveEnabled) {
              await this.saveConfiguration().catch((e) => {
                cleanupErrors.push(
                  `${connector} save: ${e instanceof Error ? e.message : String(e)}`,
                );
              });
            }
          }
          ...
          cleanupErrors.push(
            `${connector}: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }
    }
  ...
  async isMetricsEndpointVisible(): Promise<boolean> {
    ...
    const enableMetricsVisible = await this.page
      .getByText(/Enable Metrics Export/i)
      .isVisible()
      .catch(() => false);
