...
test.describe("MCP Logs", () => {
  ...
  test.describe("MCP Logs Display", () => {
    test("should display MCP logs table or getting started guide", async ({ mcpLogsPage }) => {
      ...
      const gettingStarted = await mcpLogsPage.page
  ...
  test.describe("Pagination", () => {
    ...
    test("should navigate to previous page", async ({ mcpLogsPage }) => {
      ...
      await expect.poll(() => mcpLogsPage.getCurrentPageNumber(), { timeout: 5000 }).toBe(2);
      ...
      await expect.poll(() => mcpLogsPage.getCurrentPageNumber(), { timeout: 5000 }).toBe(1);
    });
  });
  ...
  test.describe("Table Sorting", () => {
    test("should sort by timestamp", async ({ mcpLogsPage }) => {
      ...
      await expect.poll(() => mcpLogsPage.page.url(), { timeout: 5000 }).not.toBe(initialUrl);
    });
