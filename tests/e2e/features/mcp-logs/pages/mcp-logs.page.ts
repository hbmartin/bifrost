...
export class MCPLogsPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.statsCards = page
      .locator('[data-testid="mcp-stats-cards"]')
      .or(page.locator("text=Total Executions").locator("..").locator(".."));

    // Filter elements - filters are inside a popover opened by the Filters button
    this.toolNameFilter = page
      .locator('[data-testid="filter-tool-name"]')
      .or(page.locator("button").filter({ hasText: /Tool Name/i }));
    this.serverLabelFilter = page
      .locator('[data-testid="filter-server-label"]')
      .or(page.locator("button").filter({ hasText: /Server/i }));
    this.statusFilter = page
      .locator('[data-testid="filter-status"]')
      .or(page.locator("button").filter({ hasText: /Status/i }));
    this.searchInput = page
      .locator('[data-testid="filter-search"]')
      .or(page.getByPlaceholder("Search MCP logs"));
    this.dateRangePicker = page
      .locator('[data-testid="filter-date-range"]')
      .or(page.locator("button").filter({ hasText: /Last/i }));
    this.liveToggle = page
      .locator('[data-testid="live-toggle"]')
      .or(page.getByRole("button", { name: /Live updates/i }));

    // Table elements - exclude status message rows
    this.tableRows = this.logsTable
      .locator("tbody tr")
      .filter({ hasNot: page.locator("text=Listening for") })
    ...
    const paginationContainer = page
      ...
      .filter({ has: page.locator('[data-testid="next-page"]') })
      .first();
    this.paginationControls = paginationContainer;
    this.nextPageBtn = paginationContainer
      .getByRole("button", { name: "Next page" })
      .or(paginationContainer.locator('[data-testid="next-page"]'));
    this.prevPageBtn = paginationContainer
      .getByRole("button", { name: "Previous page" })
      .or(paginationContainer.locator('[data-testid="prev-page"]'));
  ...
  private async openFiltersPopover(): Promise<void> {
    ...
    await this.page.waitForSelector('[role="listbox"], [data-slot="command-list"]', {
      timeout: 5000,
    });
  }
  ...
  async selectTimePeriod(period: "1h" | "6h" | "24h" | "7d" | "30d"): Promise<void> {
    ...
    await this.page
      .waitForSelector('[role="listbox"], [role="menu"]', { timeout: 5000 })
      .catch(() => {});
  ...
  async goToNextPage(): Promise<void> {
    ...
    const limit =
      Number.parseInt(new URL(this.page.url()).searchParams.get("limit") ?? "50", 10) || 50;
    const currentOffset = Number.parseInt(
      new URL(this.page.url()).searchParams.get("offset") ?? "0",
      10,
    );
