...
test.describe("Config Settings", () => {
  ...
  test.describe("Navigation", () => {
    test("should navigate to client settings", async ({ configSettingsPage }) => {
      ...
      await expect(
        configSettingsPage.page.getByRole("heading", { name: /Client Settings/i }),
      ).toBeVisible();
    });
    ...
    test("should navigate to caching config", async ({ configSettingsPage }) => {
      ...
      await expect(
        configSettingsPage.page.getByRole("heading", { name: /Caching/i }),
      ).toBeVisible();
    });
    ...
    test("should navigate to logging config", async ({ configSettingsPage }) => {
      ...
      await expect(configSettingsPage.saveBtn).toBeVisible();
      await expect(
        configSettingsPage.page.getByRole("heading", { name: /Logging/i }),
      ).toBeVisible();
    });
    ...
    test("should navigate to performance tuning config", async ({ configSettingsPage }) => {
      ...
      await expect(configSettingsPage.saveBtn).toBeVisible();
      await expect(
        configSettingsPage.page.getByRole("heading", { name: /Performance Tuning/i }),
      ).toBeVisible();
    });
    ...
    test("should navigate to pricing config", async ({ configSettingsPage }) => {
      ...
      await expect(configSettingsPage.saveBtn).toBeVisible();
      await expect(
        configSettingsPage.page.getByRole("heading", { name: /Pricing/i }),
      ).toBeVisible();
    });
  ...
  test.describe("Pricing Config", () => {
    ...
    test.afterEach(async ({ configSettingsPage }) => {
      await configSettingsPage.goto("pricing-config");
      const canEdit = await configSettingsPage.pricingDatasheetUrlInput
        .isEditable()
        .catch(() => false);
    ...
    test("should trigger force sync", async ({ configSettingsPage }) => {
      const isForceSyncEnabled = await configSettingsPage.pricingForceSyncBtn
        .isDisabled()
        .then((d) => !d);
    ...
    test("should validate URL format", async ({ configSettingsPage }) => {
      ...
      await expect(
        configSettingsPage.page.getByText(/URL must start with http|valid URL/i),
      ).toBeVisible();
    });
  });
  ...
  test.describe("Client Settings", () => {
    ...
    test("should display async job result TTL input when available", async ({
      configSettingsPage,
    }) => {
      const isVisible = await configSettingsPage.asyncJobResultTtlInput
        .isVisible()
        .catch(() => false);
      if (isVisible) {
    ...
    test("should toggle drop excess requests", async ({ configSettingsPage }) => {
      const initialState = await configSettingsPage.getSwitchState(
        configSettingsPage.dropExcessRequestsSwitch,
      );
    ...
    test("should toggle LiteLLM fallbacks", async ({ configSettingsPage }) => {
      const initialState = await configSettingsPage.getSwitchState(
        configSettingsPage.enableLiteLLMFallbacksSwitch,
      );
    ...
    test("should toggle disable DB pings", async ({ configSettingsPage }) => {
      const initialState = await configSettingsPage.getSwitchState(
        configSettingsPage.disableDBPingsSwitch,
      );
  ...
  test.describe("Logging Settings", () => {
    ...
    test("should toggle hide deleted virtual keys in filters", async ({ configSettingsPage }) => {
      const initialState = await configSettingsPage.getSwitchState(
        configSettingsPage.hideDeletedVirtualKeysInFiltersSwitch,
      );
    ...
    test("should save and persist hide deleted virtual keys in filters toggle", async ({
      configSettingsPage,
    }) => {
      const initialState = await configSettingsPage.getSwitchState(
        configSettingsPage.hideDeletedVirtualKeysInFiltersSwitch,
    ...
    test("should display workspace logging headers textarea when available", async ({
      configSettingsPage,
    }) => {
      const isVisible = await configSettingsPage.workspaceLoggingHeadersTextarea
        .isVisible()
        .catch(() => false);
      if (isVisible) {
    ...
    test("should toggle content logging when available", async ({ configSettingsPage }) => {
      ...
      const disableContentLoggingVisible = await configSettingsPage.disableContentLoggingSwitch
        .isVisible()
        .catch(() => false);

      if (disableContentLoggingVisible) {
        const initialState = await configSettingsPage.getSwitchState(
          configSettingsPage.disableContentLoggingSwitch,
        );
    ...
    test("should save and persist content logging toggle when available", async ({
      configSettingsPage,
    }) => {
      // Check if the switch is available (depends on logs being connected)
      const disableContentLoggingVisible = await configSettingsPage.disableContentLoggingSwitch
      ...
      if (disableContentLoggingVisible) {
        ...
        const savedState = await configSettingsPage.getSwitchState(
          configSettingsPage.disableContentLoggingSwitch,
        );
