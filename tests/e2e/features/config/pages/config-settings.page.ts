...
export class ConfigSettingsPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.hideDeletedVirtualKeysInFiltersSwitch = page.getByTestId(
      "hide-deleted-virtual-keys-in-filters-switch",
    );
    this.logRetentionDaysInput = page
      .getByLabel(/Log Retention Days/i)
      .or(page.locator("#log-n-days"));
  ...
  async getCurrentSettings(configPath: string): Promise<ConfigSettingsState> {
    ...
    for (let i = 0; i < switchCount; i++) {
      ...
      if (!elId) {
        console.warn(
          `Switch at index ${i} has no id attribute — using positional fallback "switch-${i}" which may mismatch on restore`,
        );
      }
    ...
    for (let i = 0; i < inputCount; i++) {
      ...
      if (!elId) {
        console.warn(
          `Input at index ${i} has no id attribute — using positional fallback "input-${i}" which may mismatch on restore`,
        );
      }
  ...
  async getObservabilityConnectors(): Promise<string[]> {
    const connectorHeadings = this.page
      .locator("h3, h4")
      .filter({ hasText: /Datadog|New Relic|OTel|OpenTelemetry|Maxim/i });
  ...
  async toggleObservabilityConnector(connectorName: string): Promise<void> {
    const connectorSection = this.page
      .locator("div")
      .filter({ hasText: new RegExp(connectorName, "i") })
      .first();
