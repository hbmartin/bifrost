...
async function ensureTestClient001AndSendResponses(baseUrl: string): Promise<void> {
  ...
  if (listResAfter.statusCode !== 200) {
    throw new Error(
      `GET /api/mcp/clients failed after create: ${listResAfter.statusCode} ${listResAfter.body}`,
    );
  }
  const parsedAfter = JSON.parse(listResAfter.body) as
  ...
  if (clientAfter.state !== "healthy") {
    console.log(
      `MCP client "${TEST_MCP_CLIENT_NAME}" not healthy (state=${clientAfter.state}); reloading via POST /api/mcp/client/${clientId}/reconnect...`,
    );
    const reconnectRes = await httpRequest(
      baseUrl,
      "POST",
      `/api/mcp/client/${encodeURIComponent(clientId)}/reconnect`,
    );
    if (reconnectRes.statusCode < 200 || reconnectRes.statusCode >= 300) {
      ...
      console.log(
        `MCP client "${TEST_MCP_CLIENT_NAME}" uses per-call connections; reconnect not applicable, continuing.`,
      );
    }
  }
  ...
  if (listRes2.statusCode !== 200) {
    throw new Error(
      `GET /api/mcp/clients failed after reconnect: ${listRes2.statusCode} ${listRes2.body}`,
    );
  }
  const parsed2 = JSON.parse(listRes2.body) as { clients?: MCPClientItem[] } | MCPClientItem[];
  const list2 = (Array.isArray(parsed2) ? parsed2 : (parsed2.clients ?? [])).filter(
    (c) => c.config?.name === TEST_MCP_CLIENT_NAME,
  );
  ...
  for (let batch = 0; batch < count; batch += batchSize) {
    ...
    for (let i = batch; i < batchEnd; i++) {
      const body = JSON.stringify({
        model,
        messages: [
          { role: "user", content: `E2E seed message ${i + 1}: say hello in ${(i % 5) + 1} words` },
        ],
        max_tokens: 30,
      });
      promises.push(
        httpRequest(baseUrl, "POST", "/v1/chat/completions", { body })
          .then((res) => {
            if (res.statusCode >= 200 && res.statusCode < 300) {
              ...
              console.warn(
                `  Seed call ${i + 1} returned ${res.statusCode}: ${res.body.slice(0, 120)}`,
              );
            }
          })
          .catch((err) => {
...
async function globalSetup(): Promise<() => Promise<void>> {
  ...
  try {
    ...
    console.error(
      `   Ensure Bifrost is running at ${BIFROST_BASE_URL} and OPENAI_API_KEY is set for /v1/responses.`,
    );
    runMCPTeardown();
    throw error;
  }
