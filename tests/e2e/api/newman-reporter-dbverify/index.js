...
const URL_TABLE_MAP = [
  ...
  },
  {
    ...
    table: "governance_model_configs",
    idParam: 1,
    idColumn: "id",
  ...
  {
    pattern: /\/api\/governance\/budgets$/,
    table: "governance_budgets",
    idParam: null,
    idColumn: "id",
    verifyFields: [],
    bodyId: () => null,
  ...
  {
    pattern: /\/api\/governance\/rate-limits$/,
    table: "governance_rate_limits",
    idParam: null,
    idColumn: "id",
    verifyFields: [],
    bodyId: () => null,
  ...
  {
    pattern: /\/api\/logs\/histogram\/tokens$/,
    table: "logs",
    idParam: null,
    idColumn: "id",
    logsDb: true,
  ...
  {
    pattern: /\/api\/logs\/histogram\/cost$/,
    table: "logs",
    idParam: null,
    idColumn: "id",
    logsDb: true,
  ...
  {
    pattern: /\/api\/logs\/histogram\/models$/,
    table: "logs",
    idParam: null,
    idColumn: "id",
    logsDb: true,
  ...
  {
    pattern: /\/api\/logs\/histogram\/latency$/,
    table: "logs",
    idParam: null,
    idColumn: "id",
    logsDb: true,
  ...
  {
    pattern: /\/api\/logs\/histogram\/cost\/by-provider$/,
    table: "logs",
    idParam: null,
    idColumn: "id",
    logsDb: true,
  ...
  {
    pattern: /\/api\/logs\/histogram\/tokens\/by-provider$/,
    table: "logs",
    idParam: null,
    idColumn: "id",
    logsDb: true,
  ...
  {
    pattern: /\/api\/logs\/histogram\/latency\/by-provider$/,
    table: "logs",
    idParam: null,
    idColumn: "id",
    logsDb: true,
  ...
  {
    pattern: /\/api\/logs\/recalculate-cost$/,
    table: "logs",
    idParam: null,
    idColumn: "id",
    logsDb: true,
  ...
  {
    pattern: /\/api\/mcp-logs\/filterdata$/,
    table: "mcp_tool_logs",
    idParam: null,
    idColumn: "id",
    logsDb: true,
...
function parseBody(response) {
  try {
    return JSON.parse(response.stream ? response.stream.toString() : "");
  } catch {
    return null;
  }
}
...
function valuesEqual(dbVal, respVal) {
  ...
  const dbBool =
    dbVal === 1 || dbVal === true || (typeof dbVal === "string" && /^true|1$/i.test(dbVal));
  const respBool =
    respVal === 1 || respVal === true || (typeof respVal === "string" && /^true|1$/i.test(respVal));
  const dbIsBoolLike =
    dbVal === 0 ||
    dbVal === 1 ||
    dbVal === true ||
    dbVal === false ||
    (typeof dbVal === "string" && /^true|false|0|1$/i.test(dbVal));
  const respIsBoolLike =
    respVal === 0 ||
    respVal === 1 ||
    respVal === true ||
    respVal === false ||
    (typeof respVal === "string" && /^true|false|0|1$/i.test(respVal));
...
function checkJsonBlobMismatches(dbRow, jsonBlobColumn, respFields, verifyFields) {
  ...
  let blob = {};
  try {
    blob = JSON.parse(dbRow[jsonBlobColumn] || "{}");
  } catch {}
  return verifyFields
    .filter((f) => f in respFields)
...
async function verifyCreated(db, m, id, body, name) {
  ...
  const selectCols = m.jsonBlobColumn
    ? m.jsonBlobColumn
    : m.verifyFields.length
      ? m.verifyFields.join(", ")
      : m.idColumn;
  const { rows } = await db.query(`SELECT ${selectCols} FROM ${m.table} WHERE ${m.idColumn} = $1`, [
    id,
  ]);
  if (!rows.length)
    return {
      name,
      result: "FAIL",
      detail: `Row NOT found in ${m.table} where ${m.idColumn}=${id}`,
    };
...
async function verifyUpdatedSingleRow(db, table, idColumn, verifyFields, bodyFields, body, name) {
  ...
  const { rows } = await db.query(`SELECT ${selectCols} FROM ${table} LIMIT 1`, []);
  ...
  if (mm.length)
    return { name, result: "FAIL", detail: `Update NOT reflected in ${table}: ${mm.join(", ")}` };
...
async function verifyDeleted(db, m, id, name) {
  ...
  const { rows } = await db.query(
    `SELECT COUNT(*) AS cnt FROM ${m.table} WHERE ${m.idColumn} = $1`,
    [id],
  );
  const cnt = parseInt(rows[0].cnt !== undefined ? rows[0].cnt : rows[0].count, 10);
  if (cnt > 0)
    return { name, result: "FAIL", detail: `Row still exists in ${m.table}: ${m.idColumn}=${id}` };
...
async function runVerification(db, method, mapping, id, body, name) {
  switch (method) {
    case "POST":
      return verifyCreated(db, mapping, id, body, name);
    case "GET":
      return verifyExists(db, mapping, id, body, name);
    case "PUT":
    case "PATCH":
      return verifyUpdated(db, mapping, id, body, name);
    case "DELETE":
      if (mapping.deleteVerifiesExists) return verifyExists(db, mapping, id, body, name);
      return verifyDeleted(db, mapping, id, name);
    default:
      return { name, result: "SKIP", detail: `Method ${method} not verified` };
  }
}
...
async function runMultiTableVerification(db, method, mapping, body, name) {
  ...
  for (const t of tables) {
    if (method === "GET") {
      ...
      const r = await verifyUpdatedSingleRow(
        db,
        t.table,
        t.idColumn,
        t.verifyFields || [],
      ...
      results.push({
        name,
        result: "SKIP",
        detail: `Method ${method} not verified for multi-table`,
      });
    }
  }
  ...
  if (failed.length > 0)
    return { name, result: "FAIL", detail: failed.map((f) => f.detail).join("; ") };
  if (passed.length === 0)
    return results[0] || { name, result: "SKIP", detail: "No verifications run" };
...
function loadDatasheet() {
  if (!_datasheetPromise) {
    _datasheetPromise = fetchJSON(PRICING_URL).catch((e) => {
      console.warn(
        `[dbverify] could not fetch pricing datasheet (${PRICING_URL}): ${e.message}; cost-accuracy checks skipped`,
      );
      return null;
    });
  }
...
function expectedCostFromDatasheet(entry, row) {
  ...
  return (
    nonCachedPrompt * input +
    cachedRead * cacheReadRate +
    cachedWrite * cacheWriteRate +
    completion * output
  );
}
...
async function verifyCostingRequest(db, reqId, name, results, silent) {
  ...
  if (!(cost > 0 && tokens > 0)) {
    results.push({
      name,
      result: "FAIL",
      detail: `[Costing] status=${row.status} cost=$${cost} total_tokens=${tokens} — expected cost>0 && tokens>0`,
    });
    if (!silent)
      console.log(`[dbverify] costing ${reqId}: FAIL (presence) cost=$${cost} tokens=${tokens}`);
    return;
  }
  ...
  if (!silent) {
    ...
    console.log(
      `[dbverify]     tokens     : prompt=${row.prompt_tokens} completion=${row.completion_tokens} total=${tokens} cachedRead=${row.cached_read_tokens || 0}`,
    );
    console.log(`[dbverify]     logged $   : ${cost}    (logs DB row)`);
    console.log(
      `[dbverify]     expected $ : ${expectedStr}    (recomputed from getbifrost.ai/datasheet)`,
    );
...
function processRequestVerification(opts) {
  ...
  if (method === "DELETE" && !recordId) {
    ...
    const ids =
      reqBody && Array.isArray(reqBody.ids) && reqBody.ids.length > 0 ? reqBody.ids : null;
    if (ids) {
      ids.forEach((id) => {
...
function printSummary(results, dbType) {
  ...
  console.log("╚" + dline + "╝");
  console.log(
    `DB Checks: ${passed} passed, ${failed} failed, ${skipped} skipped (non-2xx or unmapped)`,
  );
  console.log("");
...
module.exports = function (newman, options) {
  ...
  let dbUrl =
    (options && (options["db-url"] || options["dbUrl"])) || process.env.BIFROST_DB_URL || null;
  if (!dbUrl) {
    dbUrl = dbUrlFromBifrostConfig(configPath);
    if (dbUrl && !silent)
      console.log(`[dbverify] Auto-detected main DB from config: ${configPath}`);
  }
  if (!dbUrl) {
    console.warn(
      "[dbverify] No main DB URL found. Provide --reporter-dbverify-db-url, BIFROST_DB_URL, or --reporter-dbverify-config. Skipping DB checks.",
    );
  }

  // Logs DB (logs_store)
  let logsDbUrl =
    (options && (options["logs-db-url"] || options["logsDbUrl"])) ||
    process.env.BIFROST_LOGS_DB_URL ||
    null;
  if (!logsDbUrl) {
    logsDbUrl = logsDbUrlFromBifrostConfig(configPath);
    if (logsDbUrl && !silent)
      console.log(`[dbverify] Auto-detected logs DB from config: ${configPath}`);
  }
  ...
  function drainCostingQueue(activeLogsDb) {
    while (earlyCostingQueue.length > 0) {
      ...
      pendingVerifications.push(
        verifyCostingRequest(activeLogsDb, item.reqId, item.name, results, silent),
      );
    }
  }
      ...
      connectionPromises.push(
        createDbClient(logsDbUrl)
          .then((client) => {
            logsDb = client;
            logsDbReady = !!client;
            if (logsDbReady && !silent)
              console.log(
                `[dbverify] Connected to logs DB (${detectDbType(logsDbUrl)}): ${safeLogsUrl}`,
              );
          ...
          .catch((e) => {
            ...
            earlyCostingQueue.forEach((item) =>
              results.push({
                name: item.name,
                result: "SKIP",
                detail: "Logs DB not connected (costing check)",
              }),
            );
    ...
    if (expectCost && /^(1|true|yes)$/i.test(String(expectCost))) {
      ...
      if (!reqId) {
        results.push({
          name,
          result: "FAIL",
          detail: "x-bf-expect-cost set but no x-request-id header to locate the log row",
        });
        return;
      }
      if (!logsDbReady || !logsDb) {
