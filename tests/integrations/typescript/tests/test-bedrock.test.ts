...
import { getConfig, getIntegrationUrl, getProviderModel } from "../src/utils/config-loader";
  ...
  describe("Streaming Chat - Client Disconnect", () => {
    ...
    it.each(testCases)(
      ...
      async ({ provider, model, vkEnabled }: ProviderModelVkParam) => {
        ...
        console.log(
          `✅ Streaming client disconnect passed for ${modelId} (${chunkCount} chunks before abort)`,
        );
      },
    );
  });
