...
export const EMBEDDINGS_SIMILAR_TEXTS = ["The cat sat on the mat.", "A feline rested on the rug."];
...
export async function collectStreamingTranscriptionContent(
...
export async function collectTextCompletionStreamingContent(
  stream: AsyncIterable<unknown>,
): Promise<string> {
  let content = "";
...
export async function collectResponsesStreamingContent(
  stream: AsyncIterable<unknown>,
): Promise<string> {
  let content = "";
...
export function assertValidAnthropicCitation(
  ...
  if (expectedType !== "web_search_result_location") {
    ...
    expect((citation as CharLocationCitation | PageLocationCitation).document_index).toBe(
      documentIndex,
    );
  }
...
export function assertValidOpenAIAnnotation(
  ...
  if (expectedType === "file_citation") {
    ...
    if (annotationObj.start_index !== undefined && annotationObj.end_index !== undefined) {
      ...
      expect(annotationObj.end_index as number).toBeGreaterThan(
        annotationObj.start_index as number,
      );
    }
