...
def _poll_video_operation(
    client: Any, operation: Any, timeout_seconds: int = 900, poll_interval_seconds: int = 10
) -> Any:
...
class TestGoogleIntegration:
                ...
                if fc.name == "get_weather":
                    ...
                    response2 = chat.send_message(
                        types.Part.from_function_response(
                            name=fc.name,
                            response={"result": "The weather in Boston is 72°F and sunny."},
                        )
                    )
                    assert_valid_chat_response(response2)
    ...
    def test_pdf_file_input(self, test_config):
        ...
        try:
            ...
            uploaded_file = direct_client.files.upload(
                file=temp_pdf_path, config=types.UploadFileConfig(display_name="test_pdf_gemini")
            )
    ...
    def test_11a_system_instruction(self, google_client, test_config, provider, model):
        ...
        assert has_answer, f"Response should contain the answer '4' or 'four'. Got: {response.text}"
        ...
        assert has_french, f"Response should contain French keywords. Got: {response2.text}"
...
Joe: Pretty good, thanks for asking."""
    ...
    def test_26_extended_thinking(self, google_client, test_config, provider, model):
        ...
        keyword_matches = sum(1 for keyword in reasoning_keywords if keyword in response_lower)
    ...
    def test_27_extended_thinking_streaming(self, google_client, test_config, provider, model):
        ...
        keyword_matches = sum(1 for keyword in library_keywords if keyword in text_lower)
        assert keyword_matches >= 3, (
    ...
    def test_28_gemini_3_pro_thought_signatures_multi_turn(self, test_config):
        ...
        response_2 = client.models.generate_content(
            ...
            config=types.GenerateContentConfig(
                tools=[calculator_tool], thinking_config=types.ThinkingConfig(thinking_level="high")
            ),
        )
                ...
                parts=[
                    types.Part(
                        function_call=types.FunctionCall(
                            name="calculate", args={"expression": "25 * 4", "operation": "multiply"}
                        ),
        ...
        if hasattr(response_1, "usage_metadata") and response_1.usage_metadata:  # noqa: SIM102
    ...
    @skip_if_no_api_key("gemini")
    @pytest.mark.parametrize(
        "model,expect_image_understood",
        [
            ("gemini-3-flash-preview", True),
        ],
    )
    def test_30_multimodal_function_response_image(
        self, test_config, model, expect_image_understood
    ):
    ...
    def test_30b_multimodal_function_response_full_workflow(
        self, test_config, model, expect_image_understood
    ):
        ...
        if not getattr(response_1, "function_calls", None):
            pytest.skip(
                f"[{model}] model did not call the tool on turn 1; cannot drive the workflow"
            )
        ...
        try:
            response = client.models.generate_content(
                ...
                contents=[
                    ...
                    IMAGE_EDIT_SIMPLE_PROMPT,  # Text content as string
                    types.Part.from_bytes(
                        data=base64.b64decode(create_simple_mask_image()), mime_type="image/png"
                    ),
                ],
            )
    ...
    def test_vertex_batch_get(self, test_config):
        ...
        try:
            ...
            print(
                f"Success: Retrieved Vertex batch job {retrieved.name}, state: {retrieved.state.name}"
            )
        finally:
            self._cleanup_vertex_job(client, job.name if job else None)
...
class TestGoogleRerank:
    ...
    @staticmethod
    def _rank(
        provider: str, model: str, payload_extra: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        payload = {
            "model": model,
            "query": RERANK_QUERY,
