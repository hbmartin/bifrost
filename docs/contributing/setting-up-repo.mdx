...
Framework vector-store tests require local backing services. Follow [Vector-store test infrastructure](/contributing/vectorstore-test-infrastructure) before running `make test-framework` or the full `make test-all` chain — that page also explains why a `make test-all` failure inside `test-framework` says nothing about the later suites in the chain.
