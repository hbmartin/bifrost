...
<Note>
The contracts use the container runtime introduced with the `v1.6.12` release line and were validated in the repository on August 18, 2026. No published Bifrost release satisfies that contract yet. Publish or update the public templates only after `.github/workflows/scripts/verify-deployment-release.sh` resolves a qualified multi-architecture release at or above the `minimum_release_tag` in `deploy/runtime-contract.json`.
</Note>
...
<Warning>
The public `blue-dark` template does not currently match the checked-in security and runtime contract, so its one-click button is deliberately withheld. Enable the button only after the live template has been updated, inspected against `deploy/railway/postgres.template-contract.json`, and its verification recorded.

Until then, reproduce the template manually from the checked-in contract rather than deploying the stale public template — with one substitution: the contract records `docker.io/maximhq/bifrost:latest` with a daily image check, which is the design for the verified public template, not for an unverified manual copy. Replace `latest` with an immutable release tag or digest you have qualified against the runtime contract, and disable the automatic image check. Do not deploy a manual reproduction while it still references `latest`; if no qualified release is published yet, there is nothing to deploy.
</Warning>
