---
title: "Render"
description: "Deploy Bifrost to Render with PostgreSQL or durable SQLite"
icon: "cloud"
---

Bifrost includes two Maxim-maintained Blueprint contracts for a single OSS replica. The checked-in files generate the encryption and administrator secrets, enable dashboard authentication, and reject anonymous inference. They reference `docker.io/maximhq/bifrost:latest`, which repository validation requires: the published one-click buttons deploy `latest` only after the release gate has qualified what it resolves to. A manual copy made before that gate passes has no such assurance, so it must substitute a qualified immutable release tag or digest, as the sections below describe.

<Note>
The Blueprints use the container runtime contract introduced with the `v1.6.12` release line. The repository contracts were validated on August 18, 2026. No published Bifrost release satisfies that contract yet: qualification requires a final multi-architecture release at or above the `minimum_release_tag` recorded in `deploy/runtime-contract.json`. Run `.github/workflows/scripts/verify-deployment-release.sh` to resolve the newest qualified release — until it reports one, the manual deployment steps below cannot be completed and there is no supported Render deployment path.
</Note>
...
Until then, copy this repository's root `render.yaml` into your own repository, replace `docker.io/maximhq/bifrost:latest` with an immutable release tag or digest that you have qualified against the runtime contract (`verify-deployment-release.sh` resolves the newest qualifying release; if it reports none, no release qualifies yet and this manual path is not available), then create and approve a Render Blueprint from that repository. Do not deploy the manual copy while it still references `latest`.
</Warning>

Once published, the button is explicitly bound to the `dev` branch and its root `render.yaml`. It provisions one Free web service and one private Free PostgreSQL 18 database. Both Bifrost stores use TLS-required PostgreSQL connections, so configuration and logs survive web-service replacement without a Bifrost disk.
...
Until then, copy `deploy/render/render-sqlite.yaml` into your own repository as the root `render.yaml`, replace `docker.io/maximhq/bifrost:latest` with an immutable release tag or digest that you have qualified against the runtime contract (`verify-deployment-release.sh` resolves the newest qualifying release; if it reports none, no release qualifies yet and this manual path is not available), then create a Render Blueprint from it. Do not deploy the manual copy while it still references `latest`.
</Warning>
...
The canonical Blueprint is maintained on `dev` at `deploy/render/render-sqlite.yaml`. After the release gate passes, publication automation creates the `render-sqlite` branch as an orphan branch containing only the generated root `render.yaml`. Maintainers then deploy and verify that branch, record the verification, and enable the button. Do not edit the generated branch directly.
