...
The canonical Blueprint is maintained on `dev` at `deploy/render/render-sqlite.yaml`. The `render-sqlite` branch does not exist yet: publication automation creates it as an orphan branch containing only the generated root `render.yaml` when the button is first verified and enabled. Do not edit the generated branch directly.
