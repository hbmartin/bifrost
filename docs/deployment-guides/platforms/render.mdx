...
Bifrost includes two Maxim-maintained Blueprint contracts for a single OSS replica. Both deploy `maximhq/bifrost:latest`, generate the encryption and administrator secrets, enable dashboard authentication, and reject anonymous inference when deployed from the checked-in files.
...
Until then, copy this repository's root `render.yaml` into your own repository, replace `docker.io/maximhq/bifrost:latest` with an immutable release tag or digest that you have qualified against the runtime contract, then create and approve a Render Blueprint from that repository. Do not deploy the manual copy while it still references `latest`.
</Warning>
...
Until then, copy `deploy/render/render-sqlite.yaml` into your own repository as the root `render.yaml`, replace `docker.io/maximhq/bifrost:latest` with an immutable release tag or digest that you have qualified against the runtime contract, then create a Render Blueprint from it. Do not deploy the manual copy while it still references `latest`.
</Warning>
