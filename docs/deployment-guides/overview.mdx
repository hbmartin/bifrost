...
{/*
  Maintainers: when the first one-click deploy target is verified (its
  last_verified / verified_release recorded in deploy/render/blueprint-verification.json
  or a deploy/railway/*.template-contract.json), validate-deployment-templates.py
  requires this page to regain an H2 heading titled "Hosted one-click choices"
  (the check looks for the heading only; the comparison content under it is
  maintained by convention). The removed section to restore lives in git
  history: `git show 4da6e0c82~1:docs/deployment-guides/overview.mdx`.
  Do not paste the heading here before a target is verified — the same check
  fails in the other direction while nothing is published.
*/}
