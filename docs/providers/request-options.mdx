...
The legacy `x-bf-prom-*` header family is removed. The gateway deliberately strips and swallows these headers: they are not forwarded to providers, copied into Bifrost context, or emitted as Prometheus labels.

Use [unified dimensions](#unified-dimensions-x-bf-dim) instead. Gateway callers send `x-bf-dim-{name}` headers, and Go SDK callers set a `map[string]string` on `BifrostContextKeyDimensions`.
