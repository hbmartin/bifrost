...
- **Image conversion**: `image_url` blocks support public HTTP(S) URLs and base64 data URLs on vision-capable models such as `command-a-vision-07-2025`
- **Raw base64 normalization**: Recognized JPEG, PNG, GIF, and WebP payloads are converted to data URLs; unknown or Cohere-unsupported image formats are rejected instead of being mislabeled as JPEG
- **Image detail**: Per-image `detail` values (`low`, `high`, or `auto`) are preserved
