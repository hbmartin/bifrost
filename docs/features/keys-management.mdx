...
A key with `weight: 0` receives no primary traffic while at least one positive-weight key is available, but it remains a reserve candidate during retry rotation. If every eligible key has weight `0`, Bifrost selects uniformly across that pool. Negative and non-finite weights supplied through the Go SDK are rejected from selection.

This all-zero behavior applies to provider API keys. Virtual Key provider configs intentionally use a different compatibility rule: an all-zero VK pool leaves provider selection unchanged. See [Governance Routing](/features/governance/routing#weighted-load-balancing).
