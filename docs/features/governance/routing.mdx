...
A provider with `weight: 0` receives no primary load-balanced traffic while a positive-weight provider is available, but remains in an automatically generated fallback chain. If every participating provider has weight `0`, Bifrost selects uniformly across that pool and uses the remaining providers as fallbacks. This supports warm-standby providers without making an all-zero configuration unroutable.
