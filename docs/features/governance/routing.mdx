...
A provider with `weight: 0` receives no primary load-balanced traffic, but remains in an automatically generated fallback chain. This supports warm-standby providers without assigning them normal request volume.
