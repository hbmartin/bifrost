...
A provider with a null or omitted `weight` is excluded from both weighted selection and the automatically generated fallback chain. An explicitly assigned `weight: 0` receives no primary load-balanced traffic while a positive-weight provider is available, but remains in the generated fallback chain; positive finite weights participate as well. If every participating provider has weight `0`, Bifrost selects uniformly across that pool and uses the remaining providers as fallbacks. This supports warm-standby providers without making an all-zero configuration unroutable.
