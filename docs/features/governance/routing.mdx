...
When you configure multiple providers on a Virtual Key, Bifrost automatically implements weighted load balancing. Each provider can be assigned a weight, and requests are distributed proportionally. The `weight` field is optional - omitting it (or setting it to `null`) excludes the provider from weighted selection and from the automatically generated fallback chain, while still allowing it to be used for direct `provider/model` requests or an explicitly configured fallback.
...
<Info>
Assigned weights are automatically normalized to a sum of 1.0 across participating providers available on the VK for the given model.
</Info>

A provider with a null or omitted `weight` is excluded from both weighted selection and the automatically generated fallback chain. An explicitly assigned `weight: 0` receives no primary load-balanced traffic but remains in the generated fallback chain when a positive-weight primary exists; positive finite weights participate in primary selection. If every assigned weight is `0`, governance skips load balancing and leaves provider selection unchanged.

This differs intentionally from provider API-key selection, where an all-zero eligible key pool is selected uniformly for backward compatibility. See [Key Management](/features/keys-management#weighted-load-balancing).
...
**How It Works:**
- **Only activated when**: At least one eligible provider has a positive weight and your request has no existing `fallbacks` array in the request body
...
5. Optionally add a weight for this provider (numeric value for weighted load balancing, or leave blank to exclude from weighted routing and automatic fallbacks while keeping the provider available for direct requests and explicitly configured fallbacks)
