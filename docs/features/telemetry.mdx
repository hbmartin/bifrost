...
<Warning>
Legacy `x-bf-prom-*` headers are no longer consumed. The gateway strips and swallows them so they are neither forwarded upstream nor added to Prometheus labels. Replace them with `x-bf-dim-*` headers.
</Warning>
