...
func main() {
    ...
    // Initialize the model catalog used for cost calculation.
    modelCatalog, err := modelcatalog.Init(ctx, nil, nil, logger)
    if err != nil {
        panic(err)
    }
    defer modelCatalog.Cleanup()
    
    // Initialize the logging plugin. Passing nil for batchStore disables
    // background batch-accounting reconciliation; gateway deployments pass
    // their ConfigStore here instead. A nil MCP catalog disables MCP costing.
    loggingPlugin, err := logging.Init(
        ctx,
        &logging.Config{},
        logger,
        store,
