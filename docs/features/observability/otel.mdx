...
func main() {
    ...
    otelPlugin, err := otel.Init(ctx, &otel.Config{
        Profiles: []*otel.Profile{{
            Enabled:       true,
            TracesEnabled: true,
            ServiceName:   "bifrost",
            CollectorURL:  schemas.NewSecretVar("http://localhost:4318/v1/traces"),
...
| `bifrost_overhead_latency_microseconds` | Histogram | Bifrost processing overhead, excluding upstream latency |
