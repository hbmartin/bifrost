...
function validateMigrationCliVersion(version) {
  ...
  console.error(`migration-cli version must be either "latest", "v1.2.3", or "v1.2.3-prerelease1"`);
  process.exit(1);
}
...
async function verifyChecksum(binaryPath, checksumUrl) {
  ...
  if (!res.ok) {
    ...
    console.error(
      `❌ Checksum file not available (${res.status}). Refusing to run an unverified binary.`,
    );
    process.exit(1);
  }
...
async function main() {
  ...
  try {
    ...
    console.error(`❌ Failed to run bifrost-migration-cli. Error:`, execError.message);
    if (execError.code) {
