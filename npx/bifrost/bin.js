...
function validateRemainingArgsForBifrostHttp(args) {
  ...
  for (const arg of args) {
    ...
    const knownList = [...BIFROST_HTTP_KNOWN_FLAGS]
      .sort()
      .map((f) => `-${f}`)
      .join(" ");
...
(async () => {
  ...
  if (VERSION === "latest") {
    ...
    if (!versionExists) {
      ...
      console.error(
        `See https://docs.getbifrost.ai/changelogs for release versions you can pass to --transport-version.`,
      );
      process.exit(1);
    }
  ...
  if (!binaryWorking) {
    ...
    if (
      ...
      (lastError.code === "ENOENT" || lastError.code === "ETXTBSY")
    ) {
